...
module.exports = class Instagram {
    ...
    if (!isHTML) {
      ...
      for (let i = 0; i < keys.length; i++) {
        ...
        if (!this.essentialValues[key]) {
          for (const cookie in src) {
            if (src[cookie].includes(key) && !src[cookie].includes(`${key}=""`)) {
              const cookieValue = src[cookie].split(';')[0].replace(`${key}=`, '');
  ...
  */
  getUserDataByUsername(username) {
    ...
    return fetch(`https://www.instagram.com/${username}`, fetch_data).then(res => res.text().then(data => {
      console.log(data);
  ...
  */
  ...
  getUserFollowers(userId, endCursor, count, followersCounter, selfSelf) {
    const self = this;
    const MAX_SAMPLE = 110;

    if (!selfSelf) { self.userIdFollowers[userId] = []; }

    if (typeof self.receivePromises[userId] !== 'undefined' && !selfSelf) { return 0; }
    ...
    return fetch(`https://www.instagram.com/graphql/query/?query_hash=56066f031e6239f35a904ac20c9f37d9&variables=${variables}`,
      ...
      if (json.status == 'ok') {
        const samples = self.userIdFollowers[userId].length;
        self.userIdFollowers[userId] = self.userIdFollowers[userId].concat(json.data.user.edge_followed_by.edges);

        if (json.data.user.edge_followed_by.page_info.has_next_page && samples < MAX_SAMPLE) {
          const end_cursor = json.data.user.edge_followed_by.page_info.end_cursor;
  ...
  */
  getUserDataById(id) {
    const query = `ig_user(${id}){id,username,external_url,full_name,profile_pic_url,biography,followed_by{count},follows{count},media{count},is_private,is_verified}`;
  ...
  */
  getFeedNextPage(json) {
    const page = json.data.user.edge_web_feed_timeline.page_info;
  ...
  */
  searchBy(searchBy, q, cursor, mediaCounter) {
    if (this.searchTypes.indexOf(searchBy) === false) { throw `search type ${searchBy} is not found`; }
